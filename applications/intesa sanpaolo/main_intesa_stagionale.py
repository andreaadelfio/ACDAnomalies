...
class Trigger:
    ...
    def run(self, thresholds: dict, type='z_score', save_anomalies_plots=True, support_vars=[], file=''):
        ...
        for face, start, stopping_time, start_datetime, stop_datetime in anomalies_list:
            merged_anomalies[start] = {face: {'changepoint': start, 'stopping_time': stopping_time, 'start_datetime': start_datetime, 'stop_datetime': stop_datetime}}
        ...
        with open(detections_file_path, 'w') as f:
            ...
            for anomaly_start, anomaly in sorted(merged_anomalies.items(), key=lambda x: int(x[0]), reverse=True):
                ...
                for face in anomaly.values():
                    if face['stopping_time'] > anomaly_end:
                        anomaly_end = face['stopping_time']
                    if face['changepoint'] < anomaly_start:
                        anomaly_start = face['changepoint']
                anomaly_end -= 1
...
df = pd.read_csv('intesa sanpaolo/ts_premi_polizze_def.csv')
...
new_df['day_of_week'] = new_df['DAT_DEFF'].dt.day_name()

# Create new columns for each day of the week for every column in y_cols
df_days_dict = {}
for col in y_cols:
    for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']:
        new_col_name = f"{col}_{day.lower()}"
        new_df[new_col_name] = new_df.apply(lambda row: row[col] if row['day_of_week'] == day else 0, axis=1)
        df_days_dict[day] = {col: new_df[['DAT_DEFF', new_col_name]][new_df[new_col_name] > 0].values}
new_df = new_df.drop(columns=y_cols + ['day_of_week'])

thresholds = {col: 50 for col in y_cols}
