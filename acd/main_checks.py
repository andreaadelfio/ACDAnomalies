...
If arguments are omitted, uses values from tslies.config:
...
from tslies.utils import Logger
...
def extract_runs_csv(raw_folder=None, workers=4):
    ...
    output_csv = os.path.join(raw_folder, 'acd_runs.csv')
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            for fut in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc='Scanning runs'):
                ...
                if 'error' in res:
                    ...
                    try:
                        with open(os.path.join(raw_folder, 'countrates_errors.txt'), 'a', encoding='utf-8') as ef:
...
if __name__ == '__main__':
    raw = '/home/andrea-adelfio/OneDrive/Workspace INFN/ACDAnomalies/applications/acd/data/LAT_ACD/raw/new_with_correct_triggs'
    try:
        csv_path = extract_runs_csv(raw, 3)
